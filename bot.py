...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "btn_refresh" or data == "btn_back_main":
        ...
        await query.edit_message_text("🔍 正在深度扫描本地视频...", parse_mode='Markdown')
        videos = scan_local_videos()
        
        if not videos:
            await query.edit_message_text(
                "❌ **未找到视频文件**\n\n"
                "已深度扫描: `Download`, `Movies` 等目录及其子文件夹。\n\n"
                "💡 **解决办法**:\n"
                "1. 确认文件后缀为 `.mp4`, `.mkv` 等。\n"
                "2. 尝试将文件移动到 `/sdcard/Download` 根目录。\n"
                "3. 再次运行 `termux-setup-storage` 刷新权限。",
    ...
    # --- 音频推流流程 ---
        ...
        await query.edit_message_text("🔍 正在深度扫描本地音频...", parse_mode='Markdown')
        audios = scan_local_audio()
        
        if not audios:
             await query.edit_message_text(
                "❌ **未找到音频文件**\n\n"
                "已深度扫描: `Music`, `Download` 等目录及其子文件夹。\n\n"
                "💡 **解决办法**:\n"
                "1. 确认文件后缀为 `.mp3`, `.flac` 等。\n"
                "2. 尝试将文件移动到 `/sdcard/Download` 根目录。\n"
                "3. 再次运行 `termux-setup-storage` 刷新权限。",
                reply_markup=get_back_keyboard(),
                parse_mode='Markdown'
